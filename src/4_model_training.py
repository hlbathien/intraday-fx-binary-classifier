...
def _run_symbol(sym: str, cfg):
    ...
    for k in HORIZONS:
        ...
        for i, sp in enumerate(splits):
            ...
            if 'joint' in best:
                import torch; torch.save(best['joint'].state_dict(), hz_dir / f'best_model_{best_name}.pt')
            else:
                with open(hz_dir / f'best_model_{best_name}.pkl','wb') as f: pickle.dump(best, f)
            with open(hz_dir / 'calibrator.pkl','wb') as f: pickle.dump(best_cal, f)
            (hz_dir/ 'thresholds.json').write_text(json.dumps({'p_star': best_thresh[0], 'theta': best_thresh[1]}, indent=2))
        (hz_dir / 'agg_metrics.json').write_text(json.dumps(_agg(fold_metrics), indent=2))
